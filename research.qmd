...
My contribution to this work, in collaboration with the lab, has two components. First, we proved that the  solution operator proposed is continuous in the underlying distribution $P_{\Lambda}^t$. Second, we generalized the approach to handle stochastic control parameters. The input distribution is now over a product space $\Lambda\times U$, corresponding to two types of parameters, calibration and control. The calibration parameters in $\Lambda$ are stochastic with unknown distribution, where as the control parameters in $U$ are stochastic with known distribution. Formally, we observe data $q_1,\cdots,q_K$ where $q_i=Q(\lambda_i, u_i)$ for $(\lambda_i, u_i)\sim P_{\Lambda\times U}^t$, where the $U$ marginal of $P_{\Lambda\times U}^t$ is known. This enables the study of a richer class of computer models using the SCP framework. For example, if we are inverting on the thermal parameters $(\Lambda)$ of a heat equation computer model, the $U$ can represent experimental controls (like placement of a burner) that have known Gaussian error.
 ...
 I'm excited to contribute to this novel area of research, which brings together fundamental probability theory, engineering applications, and computational techniques from Bayesian statistics.
